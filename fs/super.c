...
int sys_mount(char * dev_name, char * dir_name, char * type, int rw_flag)
{
	...
	if(dev_name==NULL || dir_name==NULL){
		DEBUG("nor dev name or dir name\r\n");
		return -1;
	}
	
	DEBUG("mount: dev-%s,dir-%s,type-%s\r\n",dev_name,dir_name,type);
	printk("mount: dev-%s,dir-%s,type-%s\r\n",dev_name,dir_name,type);
	if (!suser())
		return -EPERM;
	if (!(dev_i = namei(dev_name))){
		DEBUG("no dev name namei found\r\n");
		return -ENOENT;
	}
	dev = dev_i->i_rdev;
...
void mount_root(void)
{
	...
	//if (!(p=read_super(ROOT_DEV,"msdos",NULL)))
